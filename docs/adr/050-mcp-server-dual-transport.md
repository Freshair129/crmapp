...
### Tools (15 tools, 5 domains)

| Domain | Tool | Description |
|---|---|---|
| Customer | `customer.search` | ค้นหาลูกค้าด้วยชื่อ/โทรศัพท์ |
...
| Procurement | `procurement.create_po_from_bom` | สร้าง PO จาก BOM shortfall |
...
| Procurement | `procurement.approve_po` | อนุมัติ PO (write tool ⚠️) |
...
- เพิ่ม tools สำหรับ Revenue / Ad performance / Customer insights
- Remote MCP config สำหรับ Claude Desktop (HTTP transport แทน stdio)
- Rate limiting บน POST endpoint
- Tool-level permission (บาง tool เฉพาะ MANAGER+ เท่านั้น)
