...
export default function InventoryManager({ language = 'TH' }) {
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center min-h-[400px]">
                <div className="text-[#cc9d37] font-black uppercase tracking-widest animate-pulse text-xl">
                    กำลังโหลด...
                </div>
            </div>
        );
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto animate-fade-in bg-[#0c1a2f]/30 rounded-[3rem] border border-white/10 shadow-3xl">
            {/* Header */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                ...
                <button
                    ...
                    className="bg-[#cc9d37] text-[#0c1a2f] px-8 py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:scale-105 active:scale-95 transition-all flex items-center gap-3 shadow-xl shadow-[#cc9d37]/20"
                >
                    <Plus size={18} />
                    {labels.add}
                </button>
            ...
            <div className="relative mb-10 max-w-xl">
                ...
                <input
                    ...
                    className="w-full pl-14 pr-6 py-4 bg-white/5 border border-white/10 rounded-2xl text-white font-bold placeholder:text-white/20 focus:bg-white/10 focus:border-[#cc9d37]/50 transition-all outline-none shadow-xl"
                    value={search}
                                    ...
                                    <td className="px-8 py-6 text-right">
                                        <div className="flex justify-end gap-3">
                                            <button
                                                ...
                                                className="w-10 h-10 bg-white/5 hover:bg-[#cc9d37] hover:text-[#0c1a2f] text-white/40 rounded-xl transition-all flex items-center justify-center"
                                            >
                                                <Edit size={18} />
                                            </button>
                                            <button
                ...
                <div className="fixed inset-0 z-[120] flex items-center justify-center bg-[#0c1a2f]/80 backdrop-blur-md p-6 animate-fade-in">
                    <div className="bg-[#f5f8fb] w-full max-w-2xl rounded-[3rem] shadow-3xl overflow-hidden animate-scale-up border-8 border-[#0c1a2f]/10">
                        <div className="p-10 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                            ...
                            <button onClick={() => setIsModalOpen(false)} className="w-12 h-12 bg-white rounded-full shadow-md flex items-center justify-center text-slate-400 hover:text-[#0c1a2f] transition-all">
                                <X size={24} />
                            </button>
                        </div>
                        <form onSubmit={handleSave} className="p-10 grid grid-cols-2 gap-8">
                            <div className="col-span-2 space-y-2">
                                ...
                                <input required className="w-full bg-slate-100 rounded-2xl px-6 py-4 font-bold text-slate-700 focus:ring-4 focus:ring-[#cc9d37]/20 focus:bg-white transition-all outline-none" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} placeholder="e.g. Premium Thai Wagyu" />
                            </div>
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2">Pricing (THB)</label>
                                <input required type="number" className="w-full bg-slate-100 rounded-2xl px-6 py-4 font-bold text-slate-700 focus:ring-4 focus:ring-[#cc9d37]/20 focus:bg-white transition-all outline-none" value={formData.price} onChange={e => setFormData({ ...formData, price: e.target.value })} />
                            </div>
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2">Classification</label>
                                    <select
                                        className="w-full p-4 bg-white/5 border border-white/10 rounded-2xl text-white font-bold outline-none focus:border-[#cc9d37]/50 transition-all appearance-none"
                            ...
                            <div className="col-span-2 space-y-2">
                                ...
                                <input className="w-full bg-slate-100 rounded-2xl px-6 py-4 font-bold text-slate-700 focus:ring-4 focus:ring-[#cc9d37]/20 focus:bg-white transition-all outline-none" value={formData.image} onChange={e => setFormData({ ...formData, image: e.target.value })} placeholder="https://unsplash..." />
                            </div>
                            <div className="col-span-2 flex gap-4 mt-4">
                                ...
                                <button type="submit" className="flex-1 py-5 bg-[#0c1a2f] text-white rounded-2xl font-black text-xs uppercase tracking-[0.3em] shadow-2xl hover:scale-[1.02] transition-all">Save Asset</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            {/* DELETE MODAL */}
            {isDeleteModalOpen && (
                <div className="fixed inset-0 z-[130] flex items-center justify-center bg-[#0c1a2f]/90 backdrop-blur-xl p-6">
                    <div className="bg-white rounded-[3rem] p-12 text-center max-w-md animate-scale-up">
                        ...
                        <h3 className="text-3xl font-black text-[#0c1a2f] italic uppercase mb-2">Eliminate?</h3>
