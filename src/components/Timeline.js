            ...
            <div className="relative">
                ...
                <div className="space-y-6 relative z-10">
                    {sortedTimeline.map((item, idx) => {
                        ...
                        const iconMap = {
                            'ORDER': { icon: ShoppingCart, color: 'bg-orange-500 shadow-orange-100', text: 'text-orange-600', label: 'Purchase' },
                            'REDEEM': { icon: Ticket, color: 'bg-rose-500 shadow-rose-100', text: 'text-rose-600', label: 'Redemption' },
                            'CHAT': { icon: MessageSquare, color: 'bg-indigo-500 shadow-indigo-100', text: 'text-indigo-600', label: 'Inquiry' },
                            'CALL': { icon: Phone, color: 'bg-green-500 shadow-green-100', text: 'text-green-600', label: 'Contact' },
                            'TOPUP': { icon: Wallet, color: 'bg-emerald-500 shadow-emerald-100', text: 'text-emerald-600', label: 'Top-up' },
                            'EMAIL': { icon: MailOpen, color: 'bg-blue-500 shadow-blue-100', text: 'text-blue-600', label: 'Direct Mail' }
                        };

                        const typeInfo = iconMap[item.type] || { icon: CircleDot, color: 'bg-slate-400 shadow-slate-100', text: 'text-slate-600', label: item.type };
                        ...
                        return (
                            <div key={idx} className="relative pl-12 group">
                                ...
                                <div className={`absolute left-1.5 w-8 h-8 ${typeInfo.color} rounded-xl shadow-lg border-2 border-[#162A47] flex items-center justify-center z-10 group-hover:scale-110 transition-transform duration-500`}>
                                    <Icon className="text-white w-2.5 h-2.5" />
                                </div>
