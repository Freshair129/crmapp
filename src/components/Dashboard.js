...
export default function Dashboard({ customers, products, orders = [], onRefresh }) {
    ...
    customers.forEach(c => {
        const joinDateStr = c.joinDate || c.profile?.join_date || (c.timeline && c.timeline.length > 0 ? c.timeline[c.timeline.length - 1].date : null);
        if (joinDateStr) {
    ...
    const platinumMembers = customers.filter(c => (c.membershipTier || c.profile?.membership_tier) === 'PLATINUM' || (c.membershipTier || c.profile?.membership_tier) === 'GOLD');
