...
const permissionKeys = [
    { key: 'is_admin', label: 'Administrator', icon: 'fa-crown' },
    { key: 'can_access_all', label: 'Full Access', icon: 'fa-shield-alt' },
    { key: 'can_manage_orders', label: 'Orders', icon: 'fa-receipt' },
    { key: 'can_edit_inventory', label: 'Inventory', icon: 'fa-boxes' },
    { key: 'can_manage_customers', label: 'Customers', icon: 'fa-users' },
    { key: 'can_manage_employees', label: 'Employees', icon: 'fa-user-tie' },
    { key: 'can_manage_analytics', label: 'Analytics', icon: 'fa-chart-line' },
    { key: 'can_broadcast_message', label: 'Broadcast', icon: 'fa-bullhorn' },
];
    ...
    if (!filtered.length) {
        return (
            <div className="flex flex-col items-center justify-center h-96 text-white/20 gap-4">
                <i className="fas fa-users text-5xl" />
                ...
                <div className="flex gap-3">
                    ...
                    {canManage && (
                        <button className="bg-[#C9A34E] hover:bg-amber-400 text-[#0A1A2F] px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-[#C9A34E]/20 active:scale-95">
                            <i className="fas fa-user-plus mr-2" />Add
                        </button>
                    )}
                </div>
            </div>
                ...
                {emp && (
                    <div className="space-y-5">
                        ...
                        <div className="flex gap-1 bg-white/5 border border-white/8 rounded-2xl p-1">
                            {[
                                { id: 'overview', icon: 'fa-id-card', label: 'Overview' },
                                { id: 'permissions', icon: 'fa-shield-alt', label: 'Permissions' },
                                { id: 'customers', icon: 'fa-users', label: `Customers (${assignedCustomers.length})` },
                                { id: 'sales', icon: 'fa-receipt', label: `Sales (${sales.length})` },
                            ].map(tab => (
                        ...
                        {activeTab === 'overview' && (
                            <div className="space-y-5">
                                ...
                                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                                    <StatCard icon="fa-users" label="Customers" value={assignedCustomers.length} />
                                    <StatCard icon="fa-receipt" label="Orders" value={sales.length} />
                                    <StatCard icon="fa-baht-sign" label="Revenue" value={`฿${totalRevenue.toLocaleString()}`} accent />
                                    <StatCard icon="fa-star" label="Status" value={emp.status || 'Active'} sub={emp.role} />
                                </div>
                                ...
                                <div className="bg-white/5 border border-white/8 rounded-2xl p-6 space-y-4">
                                    ...
                                    {[
                                        { icon: 'fa-envelope', label: 'Email', val: emp.email },
                                        { icon: 'fa-phone', label: 'Phone', val: emp.phone || '—' },
                                        { icon: 'fa-building', label: 'Dept', val: emp.department || '—' },
                                        { icon: 'fa-facebook', label: 'Facebook', val: emp.facebookName || '—' },
                                        { icon: 'fa-id-badge', label: 'Employee ID', val: emp.employeeId },
                                    ].map(row => (
                                        <div key={row.label} className="flex items-center gap-4">
                                            <div className="w-8 h-8 rounded-xl bg-white/5 flex items-center justify-center flex-shrink-0">
                                                <i className={`fas ${row.icon} text-white/30 text-xs`} />
                                            </div>
                        ...
                        {activeTab === 'permissions' && (
                            <div className="bg-white/5 border border-white/8 rounded-2xl p-6">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    {permissionKeys.map(({ key, label, icon }) => {
                        ...
                        {activeTab === 'sales' && (
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-3">
                                    <StatCard icon="fa-receipt" label="Total Orders" value={sales.length} />
                                    <StatCard icon="fa-baht-sign" label="Total Revenue" value={`฿${totalRevenue.toLocaleString()}`} accent />
                                </div>
                                <div className="bg-white/5 border border-white/8 rounded-2xl p-6">
                                    ...
                                    {sales.length === 0
                                        ...
                                        : (
                                            <div className="space-y-2 max-h-72 overflow-y-auto custom-scrollbar pr-1">
                                                {sales.slice().sort((a, b) => new Date(b.date) - new Date(a.date)).map((o, i) => (
                    ...
                    <div className="bg-[#0D2040] border border-white/10 rounded-[2rem] w-full max-w-lg p-8 shadow-2xl">
                        ...
                        <div className="flex gap-3 mt-6">
                            ...
                            <button onClick={handleSave} disabled={isSaving}
                                ...
                                {isSaving ? <i className="fas fa-spinner fa-spin" /> : 'Save Changes'}
                            </button>
                        </div>
                    </div>
                </div>
